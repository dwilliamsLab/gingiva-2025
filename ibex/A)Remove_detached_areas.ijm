// Define input folder which is unique for each sample and contains the registration output file named 'output.ims'. 
// This folder is the source of the image data to be processed.

// Create two output directories:
// 1. SaveTarget1: This directory will store the masks of the non-overlapping regions for each Hoechst channel.
//    It will also save the overlapping regions, non-overlapping regions, and cropped ROIs.
// 2. SaveTarget2: This directory will save the final output of this script, which is a multichannel image. 
//    All processed images will be saved in one directory and named based on their respective input folder names.
...
//Briefly, process each Hoechst with background subtraction, median filter and then threshold it. Select the mask and dilate it twice. The reason that dilation is used, is in the event that there is slight difference that is only observed in the borders of the Hoechsts (nuclei overlapping in more than 80-90%).
//After adding the masks of dilated Hoechsts in ROI manager, use the "AND" function to pick overlapping areas. Now you have the ROI that is present in all Hoechsts.
selectImage("C1-output.ims"); //replace with the first Hoechst channel
run("Duplicate...", " ");
run("Median...", "radius=2"); //replace with the desired radius
run("Subtract Background...", "rolling=50"); //replace with the desired rolling ball radius
setAutoThreshold("Mean dark no-reset"); //replace with the desired thresholding method
run("Create Mask");
run("Dilate");
wait(500);
run("Dilate");
...
roiManager("Add");
//repeat upper two lines as many times as necessary to ensure that lack of registration in the cell borders is not considered. This example is for experimental batches 2 and 3.
...
run("Create Selection");
roiManager("Add");
saveAs("Tiff", SaveTarget1+"C18unregistered");
close();
//C23
selectImage("C23-output.ims");
run("Duplicate...", " ");
...
////select ROI of tissue and clear outside. If multiple ROIs combine them into one and remove them to only keep ONE combined ROI
...
//Manually crop outside the tissue area (based on the non-overlapping Hoechsts).
...
//Change order of CD3 and Thy-1 manually (they were in different order in the first experiment-IBEX5th). Stacks<Tools<Stack Sorter. They are Channel 10 and 11.
...
saveAs("Tiff", SaveTarget2+"TM327I");
close();
